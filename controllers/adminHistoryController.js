...
exports.getQueueHistory = async (req, res) => {
  try {
    ...
    const historyData = queues.map((q) => {
      ...
      const waitTime = servedTime && joinedTime 
        ? Math.round((servedTime - joinedTime) / (1000 * 60))
        : 0;

      const serviceTime = completedTime && servedTime
      ...
      return {
        ...
        joinedTime: joinedTime ? joinedTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '',
